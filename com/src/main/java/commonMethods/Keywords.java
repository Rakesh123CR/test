...
public class Keywords extends ATUReports implements OR {
	...
	public  void elementClick(WebDriver driver, String Xpath) {
		String[] values = splitXpath(Xpath);
	
		try {
			WebElement element = driver.findElement(By.xpath(values[1]));
			JavascriptExecutor js = (JavascriptExecutor) driver;
			 js.executeScript("arguments[0].setAttribute",element);
			element.click();	
			add(driver, "Click on " + values[0], LogAs.PASSED, true, values[0]);
		} catch (Exception e) {
			add1(driver, "Unable to click on " + values[0], LogAs.FAILED, true, values[0]);
	...
	public void sendText11(WebDriver driver, String Xpath){
		String[] values = splitXpath(Xpath);
	...
	public String actionType(WebDriver driver, String xpath, String keysToSend) {
		...
		try {
			...
			WebElement webElement = driver.findElement(By.xpath(values[1]));
			Actions action = new Actions(driver);
			action.moveToElement(webElement).sendKeys(webElement, keysToSend).build().perform();
			add(driver, "Type on " + values[0], keysToSend, true, values[0]);
		} catch (StaleElementReferenceException e) {
			add1(driver, "Unable to type on " + values[0], LogAs.FAILED, true, values[0]);
			Assert.fail();
	...
	public static void elementinputValuesusingActions4(WebElement element, String value) {
		try {
	...
	public String actionType1(WebDriver driver, String xpath, String keysToSend) {
		String[] values = splitXpath(xpath);
		try {
	...
	public  String combobox(WebDriver driver, String xpath, String keysToSend) {
		String[] values = splitXpath(xpath);
		try {	
			WebElement webElement = driver.findElement(By.xpath(values[1]));
			 Actions action = new Actions(driver);
	 ...
	 * String[] values = splitXpath(xpaths); try { WebElement webElement =
	 * driver.findElement(By.xpath(values[1])); webElement.sendKeys(keysToSend);
	 * add(driver, "Type on " + values[0], keysToSend, true, values[0]);
	 * 
	 * } catch (Exception e) { add1(driver, "Unable to type on " + values[0],
	 * keysToSend, true, values[0]); Assert.fail(); } return keysToSend; }
	 */
	...
	public boolean elementDisplayed(WebDriver driver, String xpath) {
		...
		try {
			...
			JavascriptExecutor js = (JavascriptExecutor) driver;
			 js.executeScript("arguments[0].setAttribute",element);//arguments[0].click();
			 add(driver, "Element '" + values[0] + "' is verified ", LogAs.PASSED, true, values[0]);
		} catch (NoSuchElementException e) {
			add1(driver, "Element is Not Present " + values[0], LogAs.FAILED, true, values[0]);
			Assert.fail();
	...
	public void scrollUsingElementTag(WebDriver driver, String xpath) {
		String[] values = splitXpath(xpath);
		try {
			WebElement element = driver.findElement(By.tagName(values[1]));
			((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", element);
			add(driver, "Scrolled to " + values[0], LogAs.PASSED, true, values[0]);
		} catch (Exception e) {
			add1(driver, "Unable to scroll " + values[0], LogAs.FAILED, true, values[0]);
	 ...
	 * try { waituntilElementVisibility(element); if (elementDisplayed(element) &&
	 * elementEnabled(element)) { elementClear(element); element.sendKeys(value);
	...
	/*
	 * public static void parentwindow(WebElement element,we) { try {
	 * 
	 * String ParentWindow = driver.getWindowHandle(); Set<String> winHandles =
	 * driver.getWindowHandles(); // Loop through all handles for(String handle:
	 * winHandles){ if(!handle.equals(ParentWindow)){
	 * driver.switchTo().window(handle); Thread.sleep(1000);
	 * System.out.println("Title of the new window: ");
	 * waituntilElementVisibility(element);
	 * System.out.println("Closing the new window..."); driver.close();
	 * 
	 * } } driver.switchTo().window(ParentWindow); } catch (Exception e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); throw new RuntimeException();
	 * 
	 * }}
	 */
	...
	/*
	 * public static void elementClear(WebElement element) { try {
	 * waituntilElementVisibility(element); if (elementDisplayed(element) &&
	 * elementEnabled(element)) { element.clear();
	 * 
	...
	/*public void waitTillElementIsClickable(WebDriver driver, String xpath,String xpath1) {
		try {
			String[] values = splitXpath(xpath);
			WebElement webElement = driver.findElement(By.xpath(values[1]));
			String[] values1 = splitXpath(xpath1);
			WebElement webElement1 = driver.findElement(By.xpath(values[1]));
			WebDriverWait wait = new WebDriverWait(driver, WaitElementSeconds);
			wait.until(ExpectedConditions.elementToBeClickable(webElement));
			add(driver, "Both", LogAs.PASSED, true, "Scroll down");
		} catch (Exception e) {
			add(driver, "Unable to wait till an element is clickable", LogAs.FAILED, true, "Scroll down");
			Assert.fail();
	...
	/*
	 * public static String getElementTextAttribute(WebElement element) { try {
	 * waituntilElementVisibility(element); String attribute =
	 * element.getAttribute("value"); return attribute; } catch (Exception e) { //
	...
	public static void selectValuesfromDD(WebElement element, String option, String value) {
		try {
	...
	/*
	 * public static void waituntilElementVisibility(driver) { try { WebDriverWait
	 * wb = new WebDriverWait(driver,60);
	 * wb.until(ExpectedConditions.visibilityOf(driver)); } catch (Exception e) { //
	 * TODO Auto-generated catch block
	 * 
	 * e.printStackTrace(); Assert.fail(); throw new RuntimeException(); } }
	 */
	 /*  public
	 * static void waituntilElementclickable(WebElement element) { try {
	 * WebDriverWait wb = new WebDriverWait(driver,50);
	 * wb.until(ExpectedConditions.elementToBeClickable(element)); } catch
	 * (Exception e) { // TODO Auto-generated catch block e.printStackTrace(); throw
	 * new RuntimeException(); } } public static void
	 * waituntilElementselectable(WebElement element) { try { WebDriverWait wb = new
	 * WebDriverWait(driver, 50);
	 * wb.until(ExpectedConditions.elementToBeSelected(element)); } catch (Exception
	 * e) { // TODO Auto-generated catch block e.printStackTrace(); throw new
	 * RuntimeException(); } }
	 * 
	 * public static void combobox(WebElement element,String value) { try {
	 * 
	 * 
	 * elementClick(element); Actions actions = new Actions(driver);
	 * actions.sendKeys(Keys.DOWN).build().perform();//press down arrow key
	 * actions.sendKeys(value).build().perform(); Actions actions1 = new
	 * Actions(driver); actions1.sendKeys(Keys.ENTER).build().perform();//press
	 * enter
	 * 
	 * 
	 * }catch (Exception e) { // TODO Auto-generated catch block
	 * e.printStackTrace(); throw new RuntimeException();
	 * 
	 * } } public static void waituntilElementVisibility(WebElement element) { try {
	 * WebDriverWait wb = new WebDriverWait(driver,60);
	 * wb.until(ExpectedConditions.visibilityOf(element)); } catch (Exception e) {
	 * // TODO Auto-generated catch block
	 * 
	 * e.printStackTrace(); Assert.fail(); throw new RuntimeException(); } } public
	 * static void waituntilElementclickable(WebElement element) { try {
	 * WebDriverWait wb = new WebDriverWait(driver,50);
	 * wb.until(ExpectedConditions.elementToBeClickable(element)); } catch
	 * (Exception e) { // TODO Auto-generated catch block e.printStackTrace(); throw
	 ...
	 * } }
	 * 
	 * public static void inputValuestoElement(WebDriver driver, String value) { try
	 * { waituntilElementVisibility(driver); if (elementDisplayed(driver) &&
	 * elementEnabled(driver)) { elementClear(driver); element.sendKeys(value);
	 * JavascriptExecutor js = (JavascriptExecutor) driver; js.
	 * executeScript("arguments[0].setAttribute('style', 'background: yellow; border: 2px solid red;');"
	 * ,element); } } catch (Exception e) { // TODO Auto-generated catch block
	 * e.printStackTrace(); throw new RuntimeException();
	 ...
	 * } }
	 * 
	 * public static void inputValuestoElement1(WebDriver driver, String value) {
	 * try { waituntilElementVisibility(element); if (elementDisplayed(driver) &&
	 * elementEnabled(driver)) { element.sendKeys(value); } } catch (Exception e) {
	 * // TODO Auto-generated catch block e.printStackTrace(); throw new
	 * RuntimeException();
	 * 
	 * } }
	 * 
	 * 
	 * public static void sendText(WebElement element, String value) { try { Actions
	 * act = new Actions(driver); act.moveToElement(element).build().perform();
	 * JavascriptExecutor js = (JavascriptExecutor) driver;
	 ...
	 * } }
	 * 
	 * public static void elementClick(WebElement element) { try {
	 * JavascriptExecutor js = (JavascriptExecutor) driver; js.
	 * executeScript("arguments[0].setAttribute('style', 'background: yellow; border: 2px solid red;');"
	 * ,element); element.click(); } catch (Exception e) { // TODO Auto-generated
	 * catch block e.printStackTrace(); throw new RuntimeException();
	 * 
	 * } }
	 * 
	 * public static String getElementText(WebElement element) { try {
	 * waituntilElementVisibility(element); String text = element.getText(); return
	 * text; } catch (Exception e) { // TODO Auto-generated catch block
	 * e.printStackTrace(); throw new RuntimeException();
	 * 
	 * } }
	 * 
	 * public static void combobox1(WebElement element,String value) { try {
	 * 
	 * 
	 * elementClick(element); Actions actions = new Actions(driver);
	 * actions.sendKeys(value).build().perform(); Actions actions1 = new
	 ...
	 * 
	 * 
	 * public static void movetoElement(WebDriver driver) { try {
	 * waituntilElementVisibility(driver); Actions ac = new Actions(driver);
	 * ac.moveToElement(driver).build().perform(); } catch (Exception e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); throw new RuntimeException();
	 * 
	 * } }
	 * 
	 * public void dragAndDrop(WebElement source, WebElement target) { try {
	 * waituntilElementVisibility(source); Actions ac = new Actions(driver);
	 * ac.dragAndDrop(source, target).build().perform(); } catch (Exception e) { //
	 * TODO Auto-generated catch block e.printStackTrace(); throw new
	 * RuntimeException();
	 * 
	 * } }
	 * 
	 * public static void doubleClick(WebDriver driver ) { try {
	 * waituntilElementVisibility(driver); Actions ac = new Actions(driver);
	 * ac.doubleClick(driver).build().perform(); } catch (Exception e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); throw new RuntimeException();
	 * 
	 * } }
	 * 
	 * public static void contextClick(WebDriver driver) { try {
	 * waituntilElementVisibility(driver); Actions ac = new Actions(driver);
	 * ac.contextClick(driver).build().perform(); } catch (Exception e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); throw new RuntimeException();
	 * } }
	 * 
	 * public static void elementClickusingActions(WebDriver driver ) { try {
	 * Actions ac = new Actions(driver); ac.lick(driver).build().perform(); } catch
	 * (Exception e) { // TODO Auto-generated catch block e.printStackTrace(); throw
	 * new RuntimeException();
	 * 
	 * }
	 * 
	 * } public static void parentwindow(WebDriver driver) { try {
	 * 
	 * String ParentWindow = driver.getWindowHandle(); Set<String> winHandles =
	 * driver.getWindowHandles(); // Loop through all handles for(String handle:
	 * winHandles){ if(!handle.equals(ParentWindow)){
	 * driver.switchTo().window(handle); Thread.sleep(1000);
	 * System.out.println("Title of the new window: ");
	 * waituntilElementVisibility(driver);
	 * System.out.println("Closing the new window..."); driver.close();
	 * 
	 * } } driver.switchTo().window(ParentWindow); } catch (Exception e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); throw new RuntimeException();
	 * 
	 * }} 
	 * public static void elementinputValuesusingActions5(WebDriver driver,
	 * String value) { try { Actions ac = new Actions(driver);
	 * ac.moveToElement(driver).sendKeys(driver, value).build().perform();
	 * ac.sendKeys(Keys.ENTER).build().perform(); } catch (Exception e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); throw new RuntimeException();
	 * 
	 * }}
	 * 
	 * public static void elementinputValuesusingActions(WebDriver driver, String
	 * value) { try { Actions ac = new Actions(driver);
	 * ac.moveToElement(driver).sendKeys(driver, value).build().perform();
	 * //ac.sendKeys(Keys.ENTER).build().perform(); } catch (Exception e) { // TODO
	 * Auto-generated catch block e.printStackTrace(); throw new RuntimeException();
	 * 
	 * }}
	 * 
	 * public static void elementinputValuesusingActions4(WebDriver driver, String
	 * value) { try { Actions ac = new Actions(driver); ac.sendKeys(driver,
	 * value).build().perform(); } catch (Exception e) { // TODO Auto-generated
	 * catch block e.printStackTrace(); throw new RuntimeException();
	 * 
	 * }}
	 * 
	 * public static void elementinputValuesusingActions2(WebDriver driver, String
	 * value) { try { Actions ac = new Actions(driver);
	 * ac.moveToElement(driver).sendKeys(driver, value).build().perform(); } catch
	 * (Exception e) { // TODO Auto-generated catch block e.printStackTrace(); throw
	 * new RuntimeException();
	 * 
	 * }
	 ...
	 * }
	 * 
	 * public void scrollUpandScrollDownusingElement(WebElement element) { try {
	 * JavascriptExecutor js = (JavascriptExecutor) driver;
	 ...
	 * } }
	 * 
	 * public static void scrollUpandDownusingCoordinates(int top, int height) { try
	 * { JavascriptExecutor js = (JavascriptExecutor) driver;
	 * js.executeScript("window.scrollBy(" + top + "," + height + ")"); } catch
	 * (Exception e) { // TODO Auto-generated catch block e.printStackTrace(); throw
	 * new RuntimeException();
	 * 
	 * } }
	 * 
	 * public static File takeScreenShot(String fileName) throws IOException { try {
	 * File f = new File(System.getProperty("user.dir") +
	 * "com\\src\\test\\resources\\Library" + fileName + ".png"); TakesScreenshot ts
	 * = (TakesScreenshot) driver; File temp = ts.getScreenshotAs(OutputType.FILE);
	 * FileUtils.copyFile(temp, f); return f;
	 * 
	 * } catch (WebDriverException e) { // TODO Auto-generated catch block
	 * e.printStackTrace(); throw new RuntimeException();
	 * 
	 * } } public void sendText11(WebDriver driver){ try{ JavascriptExecutor js =
	 * (JavascriptExecutor) driver; js.executeScript("return window.stop");
	...
	public void getWindow1(WebDriver driver, String path) {
		try {
			waitTime(driver, "5");
			Main_Window = driver.getWindowHandle();
			String[] values = splitXpath(path);
	...
	public  void parentwindow(WebDriver driver, String path) {
 ...
 String[] values = splitXpath(path);
   ...
   for(String handle: winHandles){
       ...
       Thread.sleep(1000);
       System.out.println("Title of the new window: ");
       waituntilElementVisibility(webElement);
